...
def main() -> int:
    ...
    p_bt.add_argument("--strategy", default="ema", choices=["ema", "stoch"], help="전략 선택: ema(기본) 또는 stoch")
    ...
    if args.command == "run":
        ...
        def pick_symbol_auto() -> str | None:
            ...
            try:
                ...
                top = [r for r in lst[:auto_top] if r.get("symbol") != "DOGEUSDT" and to_float(r.get("lastPrice"), 0.0) < 1.0]
                if not top:
                    return None
                ...
                for r in top:
                    ...
                    if sig_found and vol > best_vol_with_sig:
                        ...
                        best_with_sig = sym
                return best_with_sig or best_any
            except Exception:
                return None
        ...
        if not res:
            ...
            if code == 0:
                ...
                # leverage not modified: 이미 해당 설정으로 간주
                log.info("격리 %sx 이미 설정됨", leverage)
            else:
                log.error("레버리지/격리 설정 실패: %s", res)

        # TP/SL 퍼센트(수수료 고려): 진입+청산 테이커 합산을 버퍼로 반영
        # TP/SL 튜닝: 수익 극대화(트레일링 중심) 위해 TP 제거, SL은 타이트하게 설정
        # 기본 SL(가격): 0.25% + 왕복 수수료 버퍼, 최대 0.4%; 레버리지 반영 후 상한 2%
        base_sl_pct = min(0.004, 0.0025 + 2 * fee)
        sl_pct = min(base_sl_pct * leverage, 0.02)
        tp_pct = None  # TP는 브레이크이븐+ATR 트레일링으로 대체
        ...
        try:
            ...
            while True:
                ...
                if current_side is None and (time.time() - symbol_selected_at) > 600:
                ...
                if not r1 or r1.get("retCode") != 0:
                    log.error("주문/TPSL 설정 실패: %s", r1)
                else:
                    # 진입 성공 → 타이머 리셋
                    symbol_selected_at = time.time()
    ...
    if args.command == "run-stoch":
        ...
        log.info(
            "전략(StochRSI) 시작(1분봉): %s @ %sx, TP=없음(트레일링), SL=%.4f%%",
            symbol, leverage, sl_pct * 100,
        )
        try:
            import time
            from decimal import Decimal
